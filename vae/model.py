...
class VAE(object):
    ...
    def __init__(self, batch_size):
        self.x = tf.placeholder(tf.float32, [batch_size, 28, 28, 1])
        
        z_mean, z_log_var = self.encoder_network()
        eps = tf.random_normal([batch_size, VAE.latent_size], mean=0., stddev=1e-2)
        z = z_mean + tf.exp(z_log_var) * eps
        
        logits = self.decoder_network(z)
...
#         image_loss = tf.reduce_sum(image_loss, axis=-1)
    ...
    def encoder_network(self):
        net = slim.conv2d(self.x, 64, [2, 2], scope='conv1')
